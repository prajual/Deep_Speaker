...
def opposite_positive_equal_negative_batch():
    ...
    b = np.random.uniform(low=0, high=1, size=(BATCH_NUM_TRIPLETS * 3, 512))
    b[0] = -b[6]
    b[1] = -b[7]
    b[2] = -b[8]
    b[3] = -b[9]
...
def random_positive_random_negative_batch():
    # should be high
    b = np.random.uniform(low=-1, high=1, size=(BATCH_NUM_TRIPLETS * 3, 512))
    return b
...
if __name__ == '__main__':
    ...
    #assert highest_loss >= high_loss >= low_loss >= lowest_loss
